...
offline and deterministically — providers are `@weft/provider-mock` fixtures and the
...
| 06 | [`06-stdlib-patterns`](./06-stdlib-patterns) | `loopUntilDry` + `adversarialVerify` + `finalReport` from `@weft/stdlib`, composed inside one workflow |
| 07 | [`07-testing-workflows`](./07-testing-workflows) | `@weft/testing`'s `runWorkflow`: git fixtures, schema-validated mock fixtures, journal and phase assertions |
